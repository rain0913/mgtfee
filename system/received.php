...
<form method="POST" action="receivedE.php?est=<?php echo"$code";?>&start=<?php echo $_GET['start']?>&end=<?php echo $_GET['end']; ?>">
...
if (isset($_GET['end'])){
$month2 = $_GET['end'];
$sql = "SELECT RCode, Unit, ReceivedDate, SUM(Paid), Method, BCQ, GROUP_CONCAT(ManageFeeDate) FROM Total WHERE DATE(ReceivedDate) BETWEEN '$month1' AND '$month2' GROUP BY RCode, Unit, ReceivedDate, Method, BCQ ORDER BY ReceivedDate, RCode";
