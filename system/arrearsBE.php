...
if (isset($_GET['end'])){
$month = $_GET['end'];
$sql = "SELECT Unit, ManageFeeDate, Amount FROM Total WHERE DATE(DemandDate) < '$month' AND (BankInDate > '$month' OR BankInDate IS NULL ) ORDER BY Unit, ManageFeeDate";
$setRec = $mysqli->query($sql);
$name="Arrears Report(Bank)";
$date="Due to ".$month;
$columnHeader = '';
